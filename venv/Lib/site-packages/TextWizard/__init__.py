...
def main(prog=None):
    import argparse
    _parser = argparse.ArgumentParser(prog=prog, description=None)
    _parser.add_argument('t', help='txt')
    _parser.add_argument('-V', '--version', action='version',
                         version=__VERSION__)

    _args = _parser.parse_args()
    result = auto_decode(_args.t)
    if result:
...
def decode_with_base64(txt):
    ...
    try:
        ...
        result = base64.b64decode(txt.encode()).decode('utf-8', errors='ignore')
